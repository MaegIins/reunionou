  ...
  <div id="NewEvent">
    ...
    <div class="newEventForm">
      ...
      <div id="boutons">
        ...
        <button id="stop" @click="this.$router.push('/accueil')">Annuler</button>
      </div>

      <logout-comp />

      <p v-if="createEvent" class="ok">Événement créé !</p>
      ...
      <p v-if="alreadyEvent" class="error">L'événement existe déja à la même date et au même lieu</p>
    </div>
    ...
    async postEvent() {
      ...
      if (this.selectOptions === "newPlace") {
        try {
          ...
          if (error.request.status === 400 && error.response.data.message == "There is already an event at the same place and time") {
