<template>
  <div id="ListEvents">
    ...
    <div id="pending-invitations">
      ...
      <div v-for="invitation in pendingInvitations" :key="invitation.event.id_event" class="event">
        <router-link class="router" :to="'/events/' + invitation.event.id_event">
          <h2>{{ invitation.event.name }}</h2>
          <p>{{ invitation.event.description }}</p>
          <p>{{ displayCorrectDate(invitation.event.date) }} à {{ displayCorrectTime(invitation.event.date) }}</p>
        </router-link>
        <textarea v-model="invitation.comment" placeholder="Commentaire (obligatoire)"></textarea>
        <div class="invitation-actions">
          <button @click="confirmInvite(invitation.event.id_event, true, invitation.comment)">Accepter</button>
          <button @click="confirmInvite(invitation.event.id_event, false, invitation.comment)">Refuser</button>
        </div>
      </div>
    </div>
